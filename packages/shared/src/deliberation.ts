...
/**
 * Contribution-level provenance for a whole session.
 *
 * A single substituted contribution previously flagged EVERY card with a
 * session-wide "one or more contributions came from the fixture" warning, which
 * made four successful live calls look fake. Provenance is counted per
 * contribution here, and the detailed warning belongs only to the items that
 * were actually substituted.
 */
export interface ProvenanceSummary {
  /** 11 for a complete deliberation. */
  total: number;
  live: number;
  substituted: number;
  /** 'live' | 'mixed' | 'scripted' — never describe mixed as fully live. */
  mode: 'live' | 'mixed' | 'scripted';
  /** One sentence a nontechnical reader understands. */
  headline: string;
  /** How the final brief was produced, given its inputs. */
  synthesisNote: string;
  /** Roles whose contribution came from the fixture, with the stage. */
  substitutedContributions: { role: AgentRole; stage: 'opening position' | 'cross-review' }[];
}

const ROLE_WORDS: Record<AgentRole, string> = {
...
export const summarizeProvenance = (session: DeliberationSession): ProvenanceSummary => {
  const substitutedContributions: ProvenanceSummary['substitutedContributions'] = [];
  for (const position of session.initialPositions) {
    if (position.substituted) {
      substitutedContributions.push({ role: position.role, stage: 'opening position' });
    }
  }
  for (const response of session.crossReview) {
    if (response.substituted) {
      substitutedContributions.push({ role: response.role, stage: 'cross-review' });
    }
  }

  const scripted = session.source.provider === 'scripted';
  const briefSubstituted = scripted || session.errors.some((e) => e.stage === 'synthesis');
  const total =
    session.initialPositions.length + session.crossReview.length + (session.finalBrief ? 1 : 0);
  const substituted =
    substitutedContributions.length + (briefSubstituted && session.finalBrief ? 1 : 0);
  const live = Math.max(0, total - substituted);

  const mode: ProvenanceSummary['mode'] = scripted
    ? 'scripted'
    : substituted === 0
      ? 'live'
      : 'mixed';
  ...
  let headline: string;
  if (mode === 'scripted') {
    headline = 'Recorded demonstration — no live Gemini calls.';
  } else if (mode === 'live') {
    headline = `${total} of ${total} contributions generated by Gemini.`;
  } else {
    const named = substitutedContributions
      .map((c) => `the ${ROLE_WORDS[c.role]} ${c.stage}`)
      .join(' and ');
    headline =
      `${live} of ${total} contributions came from Gemini. ` +
      (named
        ? `${named.charAt(0).toUpperCase()}${named.slice(1)} used the recorded fallback because the response could not be validated.`
        : 'The final brief used the recorded fallback because the response could not be validated.');
  }

  const synthesisNote = !session.finalBrief
    ? 'No final brief yet.'
    : briefSubstituted
      ? 'Recorded fallback brief — the live synthesis could not be validated.'
      : substituted === 0
        ? 'Gemini synthesis based on all live contributions.'
        : `Gemini synthesis based on ${live - 1} live contributions and ${substituted} recorded fallback${substituted === 1 ? '' : 's'}.`;

  return { total, live, substituted, mode, headline, synthesisNote, substitutedContributions };
};
